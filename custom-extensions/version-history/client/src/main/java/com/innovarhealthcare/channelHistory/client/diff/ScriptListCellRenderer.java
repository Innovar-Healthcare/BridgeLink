...
/**
 * Cell renderer for the script list in the Global Scripts diff view.
 * Shows [M] / [A] / [D] prefix with colour-coded text.
 * <p>
 * Uses HTML coloring to bypass Substance LAF color-scheme overrides.
 * Colors match FunctionListCellRenderer and GitStatusTabPanel:
...
class ScriptListCellRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
        ...
        if (value instanceof ScriptEntry) {
            ...
            if (isSelected || entry.getChangeType() == ChangeType.UNCHANGED) {
                setText(prefix.isEmpty() ? name : prefix + " " + name);
            } else {
