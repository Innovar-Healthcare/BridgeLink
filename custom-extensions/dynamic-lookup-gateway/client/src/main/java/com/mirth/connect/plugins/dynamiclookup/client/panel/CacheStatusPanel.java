...
public class CacheStatusPanel extends JPanel {
    ...
    private void refreshUI() {
        ...
        ChartPanel lookupChartPanel = createGroupLookupsBreakdownChartPanel(cacheHits, nonHits);
    ...
    private ChartPanel createGroupLookupsBreakdownChartPanel(long cacheHits, long nonHits) {
        ...
        JFreeChart chart = ChartFactory.createBarChart("Group Lookups Breakdown", "Result Type", "Count", lookupDataset, PlotOrientation.VERTICAL, false, true, false);

        CategoryPlot plot = chart.getCategoryPlot();

        // Custom colors per category
        DrawingSupplier supplier = plot.getDrawingSupplier();
        final Paint hitColor = supplier.getNextPaint();
        final Paint missColor = supplier.getNextPaint();

        BarRenderer renderer = new BarRenderer() {
            public Paint getItemPaint(int row, int column) {
                return (column == 0) ? hitColor : missColor;
            }
        };
