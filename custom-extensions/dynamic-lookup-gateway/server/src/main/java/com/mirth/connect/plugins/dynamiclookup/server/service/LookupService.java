...
public class LookupService {
    ...
    private JsonIndexConfigurator jsonIndexConfigurator;
    ...
    public void init(LookupGroupDao groupDao, LookupValueDao valueDao, LookupAuditDao auditDao, LookupStatisticsDao statisticsDao, LookupGroupExtraDao groupExtraDao, LookupCacheManager cacheManager) {
        this.groupDao = groupDao;
        this.valueDao = valueDao;
        this.auditDao = auditDao;
        this.statisticsDao = statisticsDao;
        this.groupExtraDao = groupExtraDao;
        this.cacheManager = cacheManager;

        this.jsonIndexConfigurator = new JsonIndexConfigurator(valueDao);
    }
    ...
    public List<LookupGroup> getAllGroups() {
        List<LookupGroup> groups = groupDao.getAllGroups();
        if (groups.isEmpty()) {
            return groups;
        }

        List<LookupGroupExtra> extras = groupExtraDao.getAllGroupExtras();

        Map<Integer, LookupGroupExtra> extraByGroupId = new HashMap<>();
        for (LookupGroupExtra extra : extras) {
            extraByGroupId.put(extra.getGroupId(), extra);
        }

        for (LookupGroup group : groups) {
            group.setExtra(extraByGroupId.get(group.getId()));
        }
    ...
    public LookupGroup getGroupById(int id) {
        LookupGroup group = groupDao.getGroupById(id);
        if (group == null) {
            return null;
        }

        LookupGroupExtra extra = groupExtraDao.getByGroupId(id);
    ...
    public LookupGroup getGroupByName(String name) {
        LookupGroup group = groupDao.getGroupByName(name);
        if (group == null) {
            return null;
        }

        LookupGroupExtra extra = groupExtraDao.getByGroupId(group.getId());
        group.setExtra(extra);
     ...
     * <p>
     * This method bypasses the cache and is typically used by administrative tools or the UI to display the current value
     * and its update metadata.
     * </p>
     *
     * @param groupId the ID of the lookup group
     * @param key     the lookup key
     * @return a {@link LookupValue} containing the value and updated timestamp, or {@code null} if the key is not found in
     *         the specified group
     ...
     * <p>
     * This method always returns the value if found, regardless of how old it is. It is equivalent to calling
     * {@code getValue(groupId, key, 0)}.
     * </p>
     *
     ...
     * @param groupId    the lookup group ID
     * @param key        the lookup key
     * @param ttlSeconds if > 0, the cached value must have an updatedAt within this TTL
     ...
     * <p>
     * This method checks the cache first. Any missing values are retrieved from the database and added to the cache. Values
     * are returned regardless of how old they are.
     * </p>
     *
     * @param groupId the lookup group ID
    ...
    public void setValue(int groupId, String key, String value, String userId) {
        ...
        try {
            if (existingValue == null) {
                // Insert new value
                if (isValueJson) {
                    valueDao.insertValueJson(tableName, key, value);
                } else {
                ...
                if (isValueJson) {
                    valueDao.updateValueJson(tableName, key, value);
                } else {
    ...
    /**
     * 
     * Performs an atomic compare-and-swap (CAS) operation on a lookup value within the specified group.
     * 
     */
    ...
    /**
     * Returns a complete summary of cache metrics for the given group, including Guava stats, current entry count, and
