...
public class LookupHelper {
    ...
    /**
     * Retrieves a value from a lookup group without applying TTL validation. Always returns the best available value from
     * cache or database.
     *
    ...
    /**
     * Retrieves a value from a lookup group using TTL validation. Returns null if the cached or database value is older
    ...
    /**
     * Retrieves a value from a lookup group using TTL validation. Returns null if the value is not found, stale, or if
     * invalid TTL is provided.
     *
     * @param groupName  the name of the lookup group
     * @param key        the lookup key
    ...
    /**
     * Retrieves a value from a lookup group with a fallback default if the key is not found or null. TTL is not applied —
     * any available value (cached or from DB) will be returned.
     *
     * @param groupName    the name of the lookup group
     * @param key          the lookup key
    ...
    /**
     * Retrieves a value from a lookup group with TTL enforcement and a fallback default. If the cached or DB value is older
     * than the TTL, or not found, the default is returned.
     *
     * @param groupName    the name of the lookup group
     * @param key          the lookup key
    ...
    /**
     * Retrieves a value from a lookup group with TTL enforcement and a fallback default. If the cached or database value is
     * not found or is older than the specified TTL, the provided default value is returned.
     *
     * @param groupName    the name of the lookup group
     * @param key          the lookup key
     * @param ttlHours     hours part of TTL (must be >= 0)
     * @param ttlMinutes   minutes part of TTL (must be >= 0)
     * @param defaultValue the fallback value to return if lookup fails or the value is stale
     * @return the value if found and valid within TTL, otherwise {@code defaultValue}
     */
    ...
    /**
     * Retrieves multiple values at once from a lookup group without applying TTL validation. This method returns all
     * available values found in the cache or database, regardless of their age or update time.
     *
     * @param groupName the name of the lookup group
    ...
    /**
     * Retrieves multiple values at once from a lookup group using TTL validation. For each key, if the cached or database
     * value is older than the specified TTL, it is excluded from the result.
     *
     * @param groupName the name of the lookup group
     * @param keys      the list of keys to retrieve
     * @param ttlHours  hours part of TTL (must be >= 0)
    ...
    /**
     * Retrieves multiple values at once from a lookup group using TTL validation. For each key, if the value is not found,
     * stale, or TTL is invalid, that key is excluded from the result.
     *
     * @param groupName  the name of the lookup group
     * @param keys       the list of keys to retrieve
     * @param ttlHours   hours part of the TTL (>= 0)
     * @param ttlMinutes minutes part of the TTL (>= 0)
     * @return a map of valid key-value pairs; excludes keys with missing or stale data, or an empty map if the group does
     ...
     * <p>
     * Invoked from the Transformer layer in Mirth Connect core. Returns true if the value was successfully deleted, false
     * if the group was not found or an error occurred.
     *
     * @param groupName the name of the lookup group
     ...
     * <p>
     * Returns {@code true} if the operation completed successfully, or {@code false} if the group was not found or an error
     * occurred.
     *
     * @param groupName the name of the lookup group
     ...
     * @return a result map with fields: - ok: 'true' if successful, 'false' otherwise - groupId: the id of the group (when
     *         successful) - importedCount: number of entries imported (when successful) - errorCode / errorMessage (when
     *         failed)
     */
    ...
    /**
     * Attempts to put a lookup value in the specified group if the key does not already exist.
     *
     * @param groupName the name of the lookup group
     * @param key       the key to insert
     * @param value     the value to insert
     * @return true if the value was inserted, false if the key already exists or if an error occurred
     */
    ...
    /**
     * Atomically updates the value of a lookup key if and only if its current value matches the expected value.
     *
     * @param groupName     the name of the lookup group
     * @param key           the key whose value to conditionally update
     * @param expectedValue the value that must currently be stored for the update to occur
     * @param newValue      the new value to set if the current value matches {@code expectedValue}
     * @return {true} if the value was successfully updated; {false} if the key does not exist, the current value does not
     *         match {expectedValue}, or if an error occurred
     */
     ...
     * @return {@code true} if the value was successfully updated; {@code false} if the group or key was not found, or if an
     *         error occurred
     */
    ...
    /**
     * Search lookup values in a group by JSON field filters.
     *
     * @param groupName The name of the lookup group to search in.
     * @param filters   Key/value filters applied on JSON fields (field -> expected value).
     *
     ...
     * <p>
     * Expects a flat map with keys: name, description, version, cacheSize, cachePolicy. Returns a result map: { ok: 'true',
     * group: {...} } on success { ok: 'false', errorCode: "...", errorMessage: "..." } on failure
     *
